...
The simple approach is to set an arbitrary upper limit of detected genes and remove all cells above that point. 
...
The `scDblFinder` package provides a number of computational methods for identifying likely doublets.
...
The best approach is probably to start with a small ammount of filtering, keeping a large number of cells, and to then after evaluating cell clustering, markers, and cell type annotation to revisit filtering. 
